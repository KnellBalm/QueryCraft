...
| 기능 | 설명 | 상태 |
|------|------|------|
| PA 문제 풀이 | 정적 분석 문제 (월~금 자동 생성) | ✅ |
| 스트림 문제 풀이 | 동적 로그 분석 문제 | ✅ |
| SQL 에디터 | Monaco 기반 인터랙티브 에디터 | ✅ |
| 자동 채점 | 결과 테이블 비교 기반 채점 | ✅ |
| AI 피드백 | Gemini 기반 코드 리뷰 및 힌트 | ✅ |
| 스키마 탐색기 | 테이블/컬럼 정보 조회 | ✅ |
| 온보딩 튜토리얼 | 11단계 가이드 투어 + 하이라이트 | ✅ |
| 사용자 인증 | Google/Kakao 소셜 로그인 | ✅ |
| 레벨 시스템 | XP 기반 레벨업 (6단계) | ✅ |
| 리더보드 | 닉네임 기반 랭킹 | ✅ |
...
| 기능 | 설명 |
|------|------|
| 스케줄러 관리 | 문제/데이터 생성 스케줄 모니터링 및 수동 실행 |
| 문제 생성 이력 | PA/Stream 파일 목록, 문제 수, 생성일시 |
| API 사용량 | Gemini API 호출 수, 토큰, 예상 비용 추적 |
| 사용자 관리 | 사용자 목록, 관리자 권한 토글, 삭제 |
| 시스템 로그 | 문제 생성, 사용자 액션, 에러 로그 |
...
6. **제출** (채점)
7. **피드백 요청** (AI 리뷰)
8. **데이터 대시보드 확인** (Mixpanel/PostHog)
