...
### 1. 전 영역 KST 통합 (GCP/UTC 환경 대응)
- **문제**: 상용 서버(GCP)는 기본 환경이 UTC(세계 협정시)입니다. 내부 스케줄러뿐만 아니라 Cloud Scheduler가 호출하는 트리거 엔드포인트와 일반 사용자 API(`/problems`)도 `date.today()`를 사용하고 있어, 한국 시간(KST) 저녁 9시 이후에는 서버가 "어제"로 인식하는 문제가 있었습니다.
- **해결**:
    - `backend/common/date_utils.py`를 신설하여 서버의 시간 설정과 무관하게 항상 한국 시간(KST)을 반환하는 유틸리티를 도입했습니다.
    - 스케줄러, 트리거 엔드포인트, 문제 조회 API, 채점 서비스 등 **백엔드 전 영역**의 날짜 계산 로직을 KST 기준으로 통일했습니다.
