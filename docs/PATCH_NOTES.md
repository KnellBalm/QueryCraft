...
### 📊 사용자 성적(XP/Level) 동기화
- **Source of Truth 변경**: `stats_service.py`가 실시간 계산 대신 `public.users` 테이블에 저장된 `xp`, `level` 값을 우선 사용하도록 수정.
- **버그 수정**: `stats_service.py`에서 `pandas` 임포트 누락으로 인한 통계 에러(NameError) 해결.
- **일관성 확보**: 관리자 페이지와 일반 사용자 페이지의 성적 데이터 정합성 완벽 확보.

### 🛡️ Daily Challenge 엔진 및 안정성 보강
- **데이터 엔진 강화**: `PostgresEngine`에 `fetch_one`, `fetch_all` 등 인터페이스 호환 메소드 추가 (DuckDB와의 정밀한 호환성 확보).
- **자동 포맷 변환**: `daily_challenge_writer.py`에 구형(v1.0, List) 데이터를 신규(v2.0, Object) 포맷으로 자동 변환하는 로직 추가.
- **프론트엔드 방어**: `DailyChallenge.tsx`에 Optional Chaining 및 기본값 처리를 도입하여 데이터 미비 시에도 런타임 에러 방지.
...
- **성적 데이터 정합성 보장 (2026-01-20)**: `stats_service.py`가 `users` 테이블을 소스 오브 트루스로 사용하도록 변경하여 정밀한 성적 표시 보장.
