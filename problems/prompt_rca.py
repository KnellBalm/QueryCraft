...
RCA_SCENARIOS = {
    "commerce": [
        ...
        "신규 유저 리텐션이 Day 7 기준으로 전월 대비 40% 급락. 온보딩 퍼널 점검 필요.",
        "Paid 채널 ROAS가 급락 중. 트래픽은 정상이나 전환이 떨어짐. 특정 캠페인/소재 문제인지 확인 필요.",
        "가입 완료율이 랜딩 페이지에서 20% 하락. A/B 테스트 결과인지 버그인지 판별 필요.",
    ],
    "content": [
        ...
        "최근 가입한 유저의 D7 리텐션이 급락. 온보딩 콘텐츠 추천 로직 점검 필요.",
        "가입자 수는 늘었으나 첫 구독 전환이 30% 감소. 어느 단계에서 이탈하는지 확인 필요.",
    ],
    "saas": [
        ...
        "Trial 유저의 7일 리텐션이 전분기 대비 25% 하락. 제품 경험 품질 점검 필요.",
        "특정 유입 채널(Organic)의 가입률은 높으나 활성화율이 타 채널 대비 현저히 낮음.",
        "회원가입 퍼널 중 이메일 인증 단계에서 이탈률 급증. 메일 전송 이슈 의심.",
    ],
    "community": [
        ...
        "신규 유저의 첫 포스팅까지 걸리는 시간이 증가하며 D30 리텐션 하락.",
        "추천 채널(Referral)에서 유입된 유저의 장기 리텐션이 타 채널 대비 2배 낮음.",
        "가입 후 프로필 설정 완료율이 40% 떨어짐. UI 변경 이후 발생.",
    ],
    "fintech": [
        "해외 송금 실패율이 특정 시간대를 기점으로 급증함. 망 장애 여부 파악.",
        "계좌 연결(KYC) 완료 단계에서 유저 이탈이 평소보다 2배 높음.",
        "특정 연령대(20대)에서 투자 상품 조회 수는 높으나 실제 매수 전환이 매우 낮음.",
        "계좌 개설 후 첫 거래까지의 리텐션이 급락. 인증 절차 복잡도 증가 의심.",
        "특정 광고 채널에서 유입된 유저의 첫 입금 전환율이 50% 이상 낮음.",
        "본인인증(KYC) 퍼널에서 신분증 촬영 단계 이탈률 증가. 모바일 UI 이슈 의심.",
    ]
}
